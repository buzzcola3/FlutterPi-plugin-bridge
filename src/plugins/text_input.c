 ...
 */
static int on_set_client(struct platch_obj *object, FlutterPlatformMessageResponseHandle *responsehandle) {
    enum text_input_action input_action;
    enum text_input_type input_type;
    ...
    if ((object->json_arg.type != kJsonArray) || (object->json_arg.size != 2)) {
        return platch_respond_illegal_arg_json(responsehandle, "Expected `arg` to be an array with length 2.");
    }

    if (object->json_arg.array[0].type != kJsonNumber) {
        return platch_respond_illegal_arg_json(responsehandle, "Expected `arg[0]` to be a number");
    }

    if (object->json_arg.array[1].type != kJsonObject) {
        return platch_respond_illegal_arg_json(responsehandle, "Expected `arg[1]` to be an map.");
    }
    ...
    if (temp == NULL || (temp->type != kJsonTrue && temp->type != kJsonFalse)) {
        return platch_respond_illegal_arg_json(responsehandle, "Expected `arg[1]['autocorrect']` to be a boolean.");
    } else {
    ...
    if (temp == NULL || temp->type != kJsonObject) {
        return platch_respond_illegal_arg_json(responsehandle, "Expected `arg[1]['inputType']` to be a map.");
    }
    ...
    if (temp2 == NULL || temp2->type == kJsonNull) {
        ...
        return platch_respond_illegal_arg_json(responsehandle, "Expected `arg[1]['inputType']['signed']` to be a boolean or null.");
    }
    ...
    if (temp2 == NULL || temp2->type == kJsonNull) {
        ...
        return platch_respond_illegal_arg_json(responsehandle, "Expected `arg[1]['inputType']['decimal']` to be a boolean or null.");
    }
    ...
    return platch_respond(
        responsehandle,
        &(struct platch_obj){ .codec = kJSONMethodCallResponse, .success = true, .json_result = { .type = kJsonNull } }
    );
}

static int on_hide(struct platch_obj *object, FlutterPlatformMessageResponseHandle *responsehandle) {
    /*
     *  TextInput.hide()
...
static int on_clear_client(struct platch_obj *object, FlutterPlatformMessageResponseHandle *responsehandle) {
    /*
...
static int on_set_editing_state(struct platch_obj *object, FlutterPlatformMessageResponseHandle *responsehandle) {
    ...
    if (state->type != kJsonObject) {
        return platch_respond_illegal_arg_json(responsehandle, "Expected `arg` to be a map.");
    }

    temp = jsobject_get(state, "text");
    if (temp == NULL || temp->type != kJsonString) {
        return platch_respond_illegal_arg_json(responsehandle, "Expected `arg['text']` to be a string.");
    } else {
    ...
    if (temp == NULL || temp->type != kJsonNumber) {
        return platch_respond_illegal_arg_json(responsehandle, "Expected `arg['selectionBase']` to be a number.");
    } else {
    ...
    if (temp == NULL || temp->type != kJsonNumber) {
        return platch_respond_illegal_arg_json(responsehandle, "Expected `arg['selectionExtent']` to be a number.");
    } else {
    ...
    if (temp == NULL || (temp->type != kJsonTrue && temp->type != kJsonFalse)) {
        return platch_respond_illegal_arg_json(responsehandle, "Expected `arg['selectionIsDirectional']` to be a bool.");
    } else {
    ...
    if (temp == NULL || temp->type != kJsonNumber) {
        return platch_respond_illegal_arg_json(responsehandle, "Expected `arg['composingBase']` to be a number.");
    } else {
    ...
    if (temp == NULL || temp->type != kJsonNumber) {
        return platch_respond_illegal_arg_json(responsehandle, "Expected `arg['composingExtent']` to be a number.");
    } else {
...
static int on_show(struct platch_obj *object, FlutterPlatformMessageResponseHandle *responsehandle) {
    /*
     *  TextInput.show()
...
static int on_request_autofill(struct platch_obj *object, FlutterPlatformMessageResponseHandle *responsehandle) {
    (void) object;
    return platch_respond(
        responsehandle,
        &(struct platch_obj){ .codec = kJSONMethodCallResponse, .success = true, .json_result = { .type = kJsonNull } }
...
static int on_set_style(struct platch_obj *object, FlutterPlatformMessageResponseHandle *responsehandle) {
    (void) object;
    return platch_respond(
        responsehandle,
        &(struct platch_obj){ .codec = kJSONMethodCallResponse, .success = true, .json_result = { .type = kJsonNull } }
...
static int on_finish_autofill_context(struct platch_obj *object, FlutterPlatformMessageResponseHandle *responsehandle) {
    (void) object;
    return platch_respond(
        responsehandle,
        &(struct platch_obj){ .codec = kJSONMethodCallResponse, .success = true, .json_result = { .type = kJsonNull } }
    ...
    return platch_call_json(
        ...
        &JSONARRAY2(
            ...
            JSONOBJECT7(
                ...
                JSONSTRING(text),
                "selectionBase",
                JSONNUM(selection_base),
                "selectionExtent",
                JSONNUM(selection_extent),
                "selectionAffinity",
                JSONSTRING(selection_affinity_is_downstream ? "TextAffinity.downstream" : "TextAffinity.upstream"),
                "selectionIsDirectional",
                JSONBOOL(selection_is_directional),
                "composingBase",
                JSONNUM(composing_base),
                "composingExtent",
                JSONNUM(composing_extent)
            )
        ),
        NULL,
        NULL
...
int client_perform_action(double connection_id, enum text_input_action action) {
    char *action_str = (action == kTextInputActionNone)           ? "TextInputAction.none" :
    ...
    return platch_call_json(
        ...
        &JSONARRAY2(JSONNUM(connection_id), JSONSTRING(action_str)),
        NULL,
        NULL
    );
}
...
int client_perform_private_command(double connection_id, char *action, struct json_value *data) {
    ...
    return platch_call_json(
        ...
        &JSONARRAY2(JSONNUM(connection_id), JSONOBJECT2("action", JSONSTRING(action), "data", *data)),
        NULL,
        NULL
    );
}

int client_update_floating_cursor(double connection_id, enum floating_cursor_drag_state text_cursor_action, double x, double y) {
    return platch_call_json(
        TEXT_INPUT_CHANNEL,
        ...
        &JSONARRAY3(
            ...
            JSONOBJECT2("X", JSONNUM(x), "Y", JSONNUM(y))
        ),
        NULL,
        NULL
    );
...
int client_on_connection_closed(double connection_id) {
    return platch_call_json(TEXT_INPUT_CHANNEL, "TextInputClient.onConnectionClosed", &JSONARRAY1(JSONNUM(connection_id)), NULL, NULL);
}

int client_show_autocorrection_prompt_rect(double connection_id, double start, double end) {
    return platch_call_json(
        TEXT_INPUT_CHANNEL,
        "TextInputClient.showAutocorrectionPromptRect",
        &JSONARRAY3(JSONNUM(connection_id), JSONNUM(start), JSONNUM(end)),
        NULL,
        NULL
    );
}
...
int textin_on_xkb_keysym(xkb_keysym_t keysym) {
    ...
    switch (keysym) {
        case XKB_KEY_BackSpace: needs_sync = model_backspace(); break;
        case XKB_KEY_Delete:
        case XKB_KEY_KP_Delete: needs_sync = model_delete(); break;
        case XKB_KEY_End:
        case XKB_KEY_KP_End: needs_sync = model_move_cursor_to_end(); break;
        case XKB_KEY_Return:
